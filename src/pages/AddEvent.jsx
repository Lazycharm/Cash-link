...
export default function AddEvent() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Map form category to valid database values
      const categoryMap = {
        'cultural': 'cultural',
        'business': 'business',
        'social': 'community',  // Map social to community
        'religious': 'religious',
        'educational': 'other', // Map educational to other
        'entertainment': 'entertainment',
        'sports': 'sports',
        'other': 'other'
      };

      // Calculate ticket price from first option if not free
      const ticketPrice = !formData.is_free && formData.ticket_options?.[0]?.price 
        ? parseFloat(formData.ticket_options[0].price) 
        : null;
