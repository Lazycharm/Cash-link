...
const FeatureCard = ({ image, title, subtitle, badge, link, price, isPremium }) => (
...
export default function Home() {
  ...
  const [careerBuildUrl, setCareerBuildUrl] = useState('https://carreerbuild.com');

  // Use profile as user for display purposes
  const user = profile;

  // Fetch Career Build URL from settings
  useEffect(() => {
    const fetchCareerBuildUrl = async () => {
      try {
        const settings = await AppSettings.get();
        if (settings?.career_build_url) {
          setCareerBuildUrl(settings.career_build_url);
        }
      } catch (error) {
        console.error("Error fetching Career Build URL:", error);
      }
    };
    fetchCareerBuildUrl();
  }, []);
  ...
  useEffect(() => {
    const init = async () => {
      try {
        // Fetch Data Parallel
        const [businesses, items, jobs, allEvents, allUsers] = await Promise.all([
           Business.filter({ status: 'approved', is_promoted: true }, '-created_date', 10).catch(() => []),
           MarketplaceItem.filter({ status: 'approved' }, '-created_date', 5).catch(() => []),
           Job.filter({ status: 'approved' }, '-created_date', 5).catch(() => []),
           Event.filter({ status: 'approved' }, 'start_datetime', 20).catch(() => []),
           User.list().catch(() => [])
        ]);

        // Filter to only upcoming events and take first 5
        const now = new Date();
        const events = allEvents
          .filter(e => new Date(e.start_datetime) > now)
          .slice(0, 5);

        // Filter drivers - must be promoted to show on home page
        ...
        // Filter agents - must be promoted to show on home page
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-8 space-y-12">
      ...
      {/* 2. QUICK ACTIONS */}
      <div className="grid grid-cols-4 gap-2 sm:gap-3 md:gap-6 lg:gap-8 px-1 sm:px-2">
        <QuickAction 
            icon={HelpCircle} 
            label="UAE Help" 
      ...
      {/* Career Build and Community Buttons */}
      <div className="flex flex-col sm:flex-row justify-center items-center gap-3 sm:gap-4 mt-4 px-4">
        <motion.a
          href={careerBuildUrl}
          target="_blank"
          ...
          whileTap={{ scale: 0.95 }}
          className="inline-block bg-gradient-to-r from-blue-500 to-emerald-500 text-white font-bold py-3 px-6 sm:px-8 rounded-2xl shadow-lg hover:from-blue-600 hover:to-emerald-600 transition-all duration-300 text-base sm:text-lg hover:shadow-xl w-full sm:w-auto text-center"
        >
          Career Build
        </motion.a>
        <motion.a
          ...
          whileHover={{ scale: 1.05, y: -2 }}
          whileTap={{ scale: 0.95 }}
          className="inline-block bg-gradient-to-r from-green-500 to-emerald-500 text-white font-bold py-3 px-6 sm:px-8 rounded-2xl shadow-lg hover:from-green-600 hover:to-emerald-600 transition-all duration-300 text-base sm:text-lg hover:shadow-xl w-full sm:w-auto text-center"
        >
          Community
        </motion.a>
      </div>

      {/* 3. FEATURED SECTIONS rearranged as requested */}

      {/* Promoted Businesses */}
      {data.businesses.length > 0 && (
        <section>
          <SectionHeader title="Promoted Businesses" actionLink={createPageUrl("Businesses")} />
          <div className="flex gap-6 overflow-x-auto pb-8 -mx-4 px-4 md:mx-0 md:px-0 hide-scrollbar snap-x">
             {data.businesses.map(b => (
                 <FeatureCard 
                    ...
                    isPremium={b.subscription_status === 'active'}
                 />
             ))}
          </div>
        </section>
      ...
      {/* Promoted Cash Agents */}
      {data.agents.length > 0 && (
        <section>
          <SectionHeader title="Promoted Cash Agents" actionLink={createPageUrl("GetCash")} actionText="View All Agents" />
              ...
              <Link key={agent.id} to={createPageUrl("GetCash")} className="group">
                ...
                >
                  <div className="flex flex-col items-center text-center space-y-2">
                    <div className="relative">
                      {agent.avatar_url ? (
                        <img 
                          ...
                          alt={agent.business_name || agent.full_name}
                        ...
                        <div className="w-16 h-16 rounded-full bg-gradient-to-br from-green-400 to-emerald-500 flex items-center justify-center text-white font-bold text-xl">
                          {(agent.business_name?.[0] || agent.full_name?.[0] || 'A').toUpperCase()}
                        </div>
                      )}
                    ...
                    <div>
                      <h4 className="font-bold text-sm text-gray-900 group-hover:text-emerald-600 transition-colors line-clamp-1">{agent.business_name || agent.full_name}</h4>
      ...
      {data.drivers.length > 0 && (
        <section>
          <SectionHeader title="Promoted Drivers" actionLink={createPageUrl("GetRide")} actionText="View All Drivers" />
          <div className="flex gap-6 overflow-x-auto pb-8 -mx-4 px-4 md:mx-0 md:px-0 hide-scrollbar snap-x">
                  ...
                  <div className="p-5 space-y-3">
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="font-bold text-gray-900 group-hover:text-orange-600 transition-colors">{driver.business_name || driver.full_name}</h3>
                      ...
                      <div className="flex items-center gap-1 bg-yellow-50 px-2 py-1 rounded-full">
                        ...
                        <span className="text-xs font-semibold text-gray-700">{driver.rating?.toFixed(1) || '4.9'}</span>
                      </div>
                    </div>
