  ...
  const handleConvert = async () => {
    ...
    try {
      const response = await InvokeLLM({
        prompt: `Convert ${amount} ${fromCurrency} to AED and USD. Provide current exchange rates and converted amounts. Format the response as JSON.`,
...
export default function Welcome() {
  ...
  const handleJoinUs = async () => {
    ...
    try {
      // Use loginWithRedirect to ensure proper redirect after login
      const currentUrl = window.location.origin;
      const callbackUrl = `${currentUrl}${createPageUrl("CompleteSignup")}`;
      
      await User.loginWithRedirect(callbackUrl);
      
    } catch (error) {
      console.error("Login failed:", error);
      setLoginError("Unable to start login process. Please try again.");
  ...
  const handleCreateAccount = async () => {
    ...
    try {
      // Same as join us - Base44 handles both login and registration through Google OAuth
      const currentUrl = window.location.origin;
      const callbackUrl = `${currentUrl}${createPageUrl("CompleteSignup")}`;
      ...
      setLoginError("Unable to create account. Please try again.");
      setIsLoggingIn(false);
    }
  };
        ...
        <div className="text-center mb-12 md:mb-20 px-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center mb-8 md:mb-12 px-4">
            ...
            <Button 
              ...
              disabled={isLoggingIn}
              variant="outline"
              className="w-full sm:w-auto border-green-600 text-green-600 hover:bg-green-50 px-6 sm:px-8 py-3 sm:py-4 rounded-2xl text-base sm:text-lg font-semibold shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-200 disabled:opacity-50 disabled:transform-none"
            >
              {isLoggingIn ? (
                <>
                  <RefreshCw className="w-4 h-4 sm:w-5 sm:h-5 mr-2 animate-spin" />
          ...
          <div className="relative z-10">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Button 
                onClick={handleJoinUs}
                disabled={isLoggingIn}
                className="bg-white text-green-600 hover:bg-gray-100 px-8 py-4 rounded-2xl text-lg font-semibold shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-200 disabled:opacity-50 disabled:transform-none"
              >
                {isLoggingIn ? (
                  <>
